...
class LearnerEngine:
    ...
    ### 엑셀 파일 불러오기 ###
    def read_csv(self):
        website_df = pd.read_csv("경로\\chatbot\\learner_data\\about_website.csv", encoding="CP949")
        #print(website_df)
        return website_df
    ...
    def learn_run(self, query):
        ...
        messages.append({"role":"user", "content": 
            ...
            f"{texts_answer}\n" + 
            f"'{query}'라는 질문에 대해 질문 리스트에서 유사하거나 같은 정도가 0.7이상인(최대:1) 질문이 있다고 판단되면, 너가 최종적으로 가장 유사하다고 생각하는 질문에 대한 답변을 질문 리스트 가장 유사한 질문 번호와 매핑되는 답변리스트의 번호의 내용을 그대로 출력해줘.\n" +
            ...
            f"3. 특정 종목명(기업명) 주식을 사고 파는 행위를 직접적으로 물어보는 질문인가?\n\n" +
            ...
            f"1. '재무', '지표', '제표', '이벤트', '뉴스', '영향', '동향', '소식', '사업', '비교' 단어가 들어간 질문의 경우, 유사하지 않다고 판단한다. 이때, 답변 형식에는 'false'를 무조건 포함시킨다.\n" +
            f"2. '주가가 오를 것 같은가?'와 '주가가 내려갈 것 같은가?'와 같은 질문의 경우, 유사하지 않다고 판단한다. 이때, 답변 형식에는 'false'를 무조건 포함시킨다.\n" +
            f"3. 분석을 요청하는 질문의 경우, 유사하지 않다고 판단한다. 이때, 답변 형식에는 'false'를 무조건 포함시킨다.\n\n" +
