...
class DataManager:
    ...
    def init_method(self):
        ...
        ## 재무제표 데이터 ##
        df=pd.read_csv("경로\\chatbot\\learner_data\\financial\\2023_종합_재무제표.csv")
    ...
    def query_filter(self, query):
        ...
        만약, 해당 텍스트가 산업(업종)간 비교가 필요한 텍스트라고 판단된다면, 해당 산업(업종)에서 유명한 상장 기업 3개만 너가 임의로 선정해줘.
        ...
        request_chatgpt_count=0 # chatgpt 요청 카운팅
        target_companys=[] # 타겟 회사명들. (형식1: ['삼성전자(005930)',...] / 형식2: ['삼성전자'])
        
        # Chatgpt에게 대화 요청#
        while not bool_contain_flag: # 원하는 답변 형식이 아니면, 계속 요청 보냄.(최대 3번까지.)
            request_chatgpt_count+=1 # 요청 카운팅 갱신
            print(f"\t\t[타겟기업][chatgpt 요청 진행중 #{request_chatgpt_count}] ... ")
            # 요청
            ...
            if ("True" in result) or ("true" in result):
                ...
                except_list=['기업1', '기업2', '기업3', '기업0', '종목코드', '6자리',
                            '회사1', '회사2', '회사3', '회사0']
            ...
            if ("False" in result) or ("false" in result):
                with self.conn.cursor() as curs:
                    ...
                    bool_contain_flag=True # 만약 target_companys가 빈 리스트라면?
            
            # 요청 반복 횟수 3번 초과인 경우, 반복문 STOP
            if request_chatgpt_count>=4:
                break
            ...
            for name in target_company_names:
                ...
                if ('주가' in query) or ('가격' in query):
                    if code in pd.unique(self.df_chart['code']):
                        ...
                        tmp_df=tmp_df[['date','close','volume']].tail(10) # 수정1_12주차 : 최근 10일치
                ...
                if code in pd.unique(self.df_news['code']):
                    ...
                    tmp_df=tmp_df[['date','content']].tail(15) # 수정1_12주차 : 최근 15일치
